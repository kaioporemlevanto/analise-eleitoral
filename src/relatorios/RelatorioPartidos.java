...
public class RelatorioPartidos extends Relatorio{
    ...
    public void gerarRelatorioPartidos() {
        ...
        Map<Integer, Partido> partidosMap = eleicao.agruparCandidatosPorPartido(candidatos, eleicao.getVotacoes());
        //List<Partido> partidos = new ArrayList<>(eleicao.getPartidos());
        ...
        for (int i = 0; i < partidos.size(); i++) {
            ...
            System.out.println((i + 1) + " - " + nomePartido + " - " + numeroPartido + ", "  + df.format(totalVotos) + " votos (" + df.format(votosNominais) + " nominais e " + df.format(votosLegenda) + " de legenda), " + df.format(candidatosEleitosNoPartido) + " "+candidatosText);
        }
        ...
        for (Partido partido : partidos) {
            System.out.print((i + 1) + " - " + partido.getSiglaPartido().replace("\"", "") + " - " + partido.getNumeroPartido() + ", ");
            i+=1;
            ...
            System.out.println(primeiroColocado.getNmUrnaCandidato().replace("\"", "") + " (" + primeiroColocado.getNrCandidato() + ", " + df.format(primeiroColocado.getQtVotos()) + " "+primeiroColocadoTexto+") / " + ultimoColocado.getNmUrnaCandidato().replace("\"", "") + " (" + ultimoColocado.getNrCandidato() + ", " + df.format(ultimoColocado.getQtVotos()) + " "+ultimoColocadoTexto+")");
        }
    }
